...
class ZipfFunction(Scene):
    def construct(self):
        ...
        text = Text("LET'S TRY TO PROVE THE\n\n", font_size=40, color=YELLOW_B).scale(1).shift(UP)
        ...
        self.play(FadeIn(text),FadeIn(text2) ,run_time=2)
        self.wait()

        self.play(FadeOut(text),FadeOut(text2) ,run_time=1.5)

        text3 = Text("THEOREM STATEMENT :\n", font_size=40, color=MAROON_C).scale(0.9).shift(UP*2)
        ...
        self.play(
           formula_box.animate.scale(0.6 ).shift(UP*4.2),
            formula8.animate.shift(UP * 4.2).scale(0.5)
        )
        ...
        text3 = Text(" ➣Then we have L - log M symbols left per molecule to encode data.",
                     slant=ITALIC, font_size=40, color=WHITE).scale(0.6)
        self.play(FadeIn(text3), run_time=1.5)
        self.wait()

        text4 = Text("➣ ERASURE CHANNEL - when a molecule is not drawn (Ni = 0) with probability q0.", slant=ITALIC, font_size=40, color=WHITE).scale(0.5).shift(DOWN)
        self.play(FadeIn(text4), run_time=1.5)
        self.wait()
        ...
        equation = MathTex(
            ...
            font_size=72,
            color=PURPLE_B
        ).scale(0.6).shift(DOWN*2.2)

        self.play(FadeIn(equation))
        self.wait(2)

        self.play(FadeOut(text2),FadeOut(text3),FadeOut(text4),FadeOut(equation),FadeOut(formula8),FadeOut(formula_box))
        ...
        text_side= Text("We know that", slant=ITALIC, font_size=40, color=WHITE).scale(0.5).shift(UP + RIGHT*3)
        ...
        formula_box_side = SurroundingRectangle(VGroup(text_side,formula_side,formula2,formula3,formula4), buff=0.2)
        ...
        formula_box_side2 = SurroundingRectangle(formula3, buff=0.2,color=MAROON_C)
